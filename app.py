...
def predict_image(image, model, transformations, class_names):
    im = transformations(image).unsqueeze(0).to(device)
    with torch.no_grad():
        preds = model(im)
        pred_class = torch.argmax(preds, dim=1).item()
    class_name = class_names[pred_class]
...
def predict_disease():
    ...
    try:
        ...
        predicted_class = predict_image(image, model, tfs, list(classes.keys()))
        crop = predicted_class.split('_')[0]
        disease = ' '.join(predicted_class.split('_')[1:])
        return jsonify({"crop": crop, "disease": disease.strip()})
    except Exception as e:
        return jsonify({"error": f"An error occurred: {str(e)}"}), 500
...
@app.route('/')
def home():
    return "Welcome to the Crop Disease Detection and Recommendation System!"
